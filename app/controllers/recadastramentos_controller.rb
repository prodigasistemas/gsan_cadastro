class RecadastramentosController < ApplicationController
  def index
    ...
    if params[:query]
      ...
      @colunas_atualizacoes_cadastral = ColunaAtualizacaoCadastral.com_relacionamentos.order(:atualizacao_cadastral_id).page(@page).per(@per)
    end

    if @colunas_atualizacoes_cadastral.any?
      render json: { entidades: @colunas_atualizacoes_cadastral.map {|c| Recadastramento::ColunaDto.new(c) }, total: @total, page: @page, per_page: @per }, status: :ok
    else
